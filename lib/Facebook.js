...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.replyWithTyping = function(src, resp, cb) {
            ...
            var avgWPM = 85;
            var avgCPM = avgWPM * 7;
            var typingLength = 100;
            
            if (typeof(resp) == 'string') {
                text = resp;
                typingLength = Math.min(Math.floor(text.length / (avgCPM / 60)) * 1000, 5000);
            } else {
                text = resp.text;
            }
        ...
        if (obj.entry) {
            for (var e = 0; e < obj.entry.length; e++) {
                for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                    ...
                    console.log("facebook_message facebook_message facebook_message facebook_message facebook_message ");
                    console.log(facebook_message);
                    console.log(facebook_message.sender);
                    console.log("facebook_message facebook_message facebook_message facebook_message facebook_message ");
